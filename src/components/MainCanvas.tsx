...
export function MainCanvas() {
  ...
  useEffect(() => {
    ...
    const renderPage = async () => {
      try {
        const page = await (pdfDoc as any).getPage(currentPage + 1);
        if (cancelled) return;
  ...
  }, [pdfDoc, currentPage, zoom, zoomMode]);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      if (pdfDoc) {
        // Re-trigger render via state dependency (zoom won't change but forces re-render)
        const canvas = canvasRef.current;
        if (canvas) {
          setCanvasSize({ width: canvas.width, height: canvas.height });
        }
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (dragging === 'split-line') {
        // 分割線ドラッグ: splitOffsetPercent を更新
    ...
    >
      ...
      {!pdfDoc && !isLoading && (
        <div style={{
          ...
          <p>PDF をドラッグ&ドロップするか、<br />ヘッダーの「見開きテスト.pdf を読込」をクリック</p>
        </div>
      )}
      ...
      {/* Crop overlay (only when PDF is loaded and canvas has size) */}
